...
class WhatsAppClient {
    ...
    if(isRegistered.error){
        return {
            error:true,
            message:'the format of the number is not correct',
            developerMessage:isRegistered.message,
        };
    }else{
        return {
            error:false,
        ...
        try {
            ...
            const chatList = await Promise.all(
                fetchedChats.map(async chat => {
                    ...
                    try {
                        // console.log(`Failed to fetch profile image for chat ${chat.id._serialized}:, error`);
                        ...
                        console.warn(`Failed to fetch profile image for chat ${chat.id._serialized}:, error: ${error}`);
                    }
                    ...
                    if (!chat.isGroup) {
                        try {
                            commonGroups = await this.client.getCommonGroups(chat.id._serialized);
                        } catch (error) {
                            console.warn(`Failed to fetch common groups for chat ${chat.id._serialized}: error: ${error}`);
                        }
                    }        
                    if (chat.lastMessage) {
    ...
    async  getChatMediaLinksDocs(chatId) {
        try {
    ...
    // console.log('i am herererererer');
            ...
            for (const message of messages) {
                ...
                if (message.body) {
                    ...
                    const foundLinks = message.body.match(urlRegex);
                    if (foundLinks) {
                        links.push({
                            messageId: message.id._serialized,
                            urls: foundLinks,
                ...
                if (message.type === "document") {
                    ...
                    if (document) {
                        ...
                        fs.writeFileSync(filePath, Buffer.from(document.data, 'base64'));
                        mediaUrl = `/media/${filename}`;    
                        documents.push({
                            messageId: message.id._serialized,
            ...
            return { mediaFiles, links, documents };
        } catch (error) {
